...
public class ComponentHeading extends Component {
    ...
    int ledColor = ColorUtil.YELLOW;
    int numDigits = 3;

    public ComponentHeading(GameWorld gw)
    {
        this.gw = gw;
        try
        {
    ...
    {
        int h = gw.player.getHeading();
        headingDigits[0] = digitImages[Math.abs(h)/100];
        headingDigits[1] = digitImages[(Math.abs(h)%100)/10];
        headingDigits[2] = digitImages[Math.abs(h)%10];
    }
    ...
    public void paint(Graphics g)
    {
        ...
        g.drawImage(headingDigits[0], displayX + 0 * displayDigitWidth, displayY, displayDigitWidth, displayDigitHeight);
        g.drawImage(headingDigits[1], displayX + 1 * displayDigitWidth, displayY, displayDigitWidth, displayDigitHeight);
        g.drawImage(headingDigits[2], displayX + 2 * displayDigitWidth, displayY, displayDigitWidth, displayDigitHeight);
